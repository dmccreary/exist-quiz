...
module namespace template = "http://www.danmccreary.com/template";
...
declare variable $temp:app-home := '/db/nuemeta/apps/template';
...
:)
declare function template:prev-next-controls($start as xs:integer, $num as xs:integer, $count as xs:integer, $category as xs:string) as node()* {
 ...
 declare function template:category-buttons($cols as xs:integer) as node()* {
